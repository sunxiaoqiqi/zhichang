...
export async function GET() {
  ...
  const rows = await getDb().select({ id: users.id, account: users.account, role: users.role, accessPlan: users.accessPlan, status: users.status, mustChangePassword: users.mustChangePassword, note: users.note, createdAt: users.createdAt, updatedAt: users.updatedAt }).from(users).orderBy(desc(users.createdAt));
...
export async function POST(request: Request) {
  ...
  return Response.json({ user: { id, account: normalized, role, accessPlan: "free" }, temporaryPassword }, { status: 201 });
}
