...
type AnalyticsData = {
  ...
  rollups: { id: string; userId: string; account: string; browser: string; os: string; deviceType: string; note: string; loginCount: number; activeSeconds: number; lastLoginAt: string | null }[];
  events: { id: string; userId: string | null; account: string; accountAtLogin: string; success: boolean; location: string; ip: string; occurredAt: string; deviceId: string | null }[];
  sessions: { id: string; userId: string; account: string; device: string; location: string; ip: string; startedAt: string; lastHeartbeatAt: string; endedAt: string | null; activeSeconds: number }[];
...
export default function AnalyticsPage() {
  ...
  return <main className="adminShell">
    ...
    <section className="adminMain analyticsMain">
      <header><div><small>VERSION 3 · INSIGHTS</small><h1>数据分析</h1><p>以“账户 + 设备”为唯一口径，查看登录、地域与有效使用时长。</p></div><div className="adminPageHeaderActions"><a className="adminHeaderAction" href="/api/admin/export?type=logins">导出登录 CSV</a><strong>最近更新 {new Date().toLocaleTimeString("zh-CN", { hour: "2-digit", minute: "2-digit" })}</strong></div></header>
      ...
      <section className="analyticsSection"><header><div><small>ACCOUNT + DEVICE</small><h2>账户设备汇总</h2></div><div className="sectionActions"><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="搜索账户或设备" /><button className={sort === "logins" ? "active" : ""} onClick={() => setSort("logins")}>按次数</button><button className={sort === "duration" ? "active" : ""} onClick={() => setSort("duration")}>按时长</button></div></header>
        <div className="adminTable"><table><thead><tr><th>账号名 / 用户编号</th><th>设备</th><th>成功登录</th><th>有效时长</th><th>最后登录</th></tr></thead><tbody>{rollups.map((row) => <tr key={row.id}><td><strong>{row.account}</strong><small title={row.userId}>用户编号 {row.userId.slice(0, 8)}… · {row.note || "未备注"}</small></td><td>{row.browser} · {row.os}</td><td><strong>{row.loginCount} 次</strong></td><td>{formatDuration(row.activeSeconds)}</td><td>{formatDateTime(row.lastLoginAt)}</td></tr>)}{!rollups.length && <tr><td className="tableEmpty" colSpan={5}>当前时段没有账户设备数据</td></tr>}</tbody></table></div>
      </section>
      <section className="analyticsSection"><header><div><small>LOGIN DETAILS</small><h2>登录明细</h2></div><span>最多展示最近 500 条</span></header>
        <div className="adminTable"><table><thead><tr><th>登录时间</th><th>账号名 / 用户编号</th><th>结果</th><th>登录地</th><th>IP</th></tr></thead><tbody>{events.map((event) => <tr key={event.id}><td>{formatDateTime(event.occurredAt)}</td><td><strong>{event.account}</strong>{event.userId && <small title={event.userId}>用户编号 {event.userId.slice(0, 8)}…{event.accountAtLogin !== event.account ? ` · 登录时账号：${event.accountAtLogin}` : ""}</small>}</td><td><i className={event.success ? "active" : "disabled"}>{event.success ? "成功" : "失败"}</i></td><td>{event.location}</td><td><code>{event.ip || "—"}</code></td></tr>)}{!events.length && <tr><td className="tableEmpty" colSpan={5}>当前时段没有登录明细</td></tr>}</tbody></table></div>
      ...
      <section className="analyticsSection"><header><div><small>SESSION DETAILS</small><h2>每次使用时长</h2></div><span>登录后每个会话单独记录</span></header>
        <div className="adminTable"><table><thead><tr><th>登录时间</th><th>账号名 / 设备</th><th>登录地</th><th>有效时长</th><th>会话状态</th></tr></thead><tbody>{sessions.map((session) => <tr key={session.id}><td>{formatDateTime(session.startedAt)}</td><td><strong>{session.account}</strong><small title={session.userId}>用户编号 {session.userId.slice(0, 8)}… · {session.device}</small></td><td>{session.location}</td><td><strong>{formatDuration(session.activeSeconds)}</strong></td><td><i className={session.endedAt ? "disabled" : "active"}>{session.endedAt ? `结束于 ${formatDateTime(session.endedAt)}` : "进行中"}</i></td></tr>)}{!sessions.length && <tr><td className="tableEmpty" colSpan={5}>当前时段没有使用会话</td></tr>}</tbody></table></div>
