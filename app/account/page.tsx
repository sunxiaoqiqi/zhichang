...
export default function AccountPage() {
  const [user, setUser] = useState<{ id: string; account: string }>();
  const [message, setMessage] = useState("");
  const [busy, setBusy] = useState(false);
  useEffect(() => { fetch("/api/auth/me").then((response) => response.json()).then((result: { user?: { id: string; account: string } }) => setUser(result.user)); }, []);
